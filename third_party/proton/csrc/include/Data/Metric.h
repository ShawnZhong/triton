...
public:
  ...
  CycleMetric(uint64_t startCycle, uint64_t endCycle, uint64_t duration,
              ...
              uint64_t preFinalTime, uint64_t postFinalTime, uint64_t initCycle,
              uint64_t preFinalCycle)
      : CycleMetric() {
    ...
    this->values[InitCycle] = initCycle;
    this->values[PreFinalCycle] = preFinalCycle;
  }
...
private:
  ...
  const static inline std::string VALUE_NAMES[CycleMetricKind::Count] = {
      ...
      "normalized_cycles", "kernel_id",       "kernel_name",
      "block_id",          "processor_id",    "unit_id",
      "device_id",         "device_type",     "time_shift_cost",
      "init_time",         "pre_final_time",  "post_final_time",
      "init_cycle",        "pre_final_cycle",
  };
};
